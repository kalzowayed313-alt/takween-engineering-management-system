      ...
      <div className={`min-h-screen flex transition-colors duration-300 ${isDarkMode ? 'dark bg-slate-950 text-white' : 'bg-slate-50 text-slate-900'}`} dir="rtl">
        ...
        <main className={`flex-1 ${isSidebarOpen ? 'mr-64' : 'mr-20'} p-8 transition-all duration-300`}>
          ...
          <Routes>
            ...
            <Route path="/projects" element={<ProjectsList projects={projects} tasks={tasks} employees={employees} setProjects={setProjects} setTasks={setTasks} currentUser={currentUser} onAccessDeny={() => setIsPermissionModalOpen(true)} />} />
        ...
        <TaskModal isOpen={isTaskModalOpen} onClose={() => setIsTaskModalOpen(false)} onAdd={(task) => setTasks(prev => [task, ...prev])} employees={employees} projects={projects} kpiRules={kpiRules} />
        ...
        {isManager && (
          <button onClick={() => setIsTaskModalOpen(true)} className="fixed bottom-8 left-8 w-16 h-16 bg-blue-600 text-white rounded-full shadow-2xl flex items-center justify-center hover:scale-110 transition-all z-40 group">
            <PlusCircle size={32} />
          </button>
        )}
      </div>
