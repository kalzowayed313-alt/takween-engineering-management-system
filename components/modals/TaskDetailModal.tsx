...
const TaskDetailModal: React.FC<TaskDetailModalProps> = ({ 
  ...
  /**
   * الحل السحري: استخدام Google Docs Viewer لعرض الملفات
   * هذا المحرك يحول الـ PDF إلى رابط ويب يقرأه المتصفح دون حظر
   */
  const getSafePreviewUrl = (file: Attachment) => {
    let url = file.url;

    // إذا كان الملف من Google Drive، نحوله أولاً لرابط معاينة
    ...
    // للملفات التي لها رابط مباشر (Online)، نغلفها بمحرك Google Docs
  ...
  const handlePreview = (file: Attachment) => {
    ...
    if (file.url.startsWith('data:')) {
      ...
      if (blob) {
        ...
        setDisplayUrl(url); // الـ Blob سيعرض مباشرة لأنه محلي
        setPreviewFile(file);
      }
    }
  };
  ...
  return (
    <>
          ...
          <div className="flex-1 flex overflow-hidden">
            ...
            <div className={`overflow-y-auto custom-scrollbar p-8 space-y-10 transition-all ${previewFile ? 'w-[380px] border-l border-slate-100 dark:border-slate-800' : 'w-full'}`}>
              ...
              <section className="space-y-6">
                <div className="flex items-center justify-between border-b border-slate-50 dark:border-slate-800 pb-4">
                  <div className="flex items-center gap-2">
                     ...
                     <h4 className="text-sm font-black text-slate-800 dark:text-white uppercase tracking-tight">المرفقات الهندسية</h4>
                  </div>
                </div>
                ...
                <div className="space-y-3">
                  {formData.attachments && formData.attachments.length > 0 ? (
                      ...
                      >
                        <div className="flex items-center gap-4 min-w-0">
                          ...
                          <div className="min-w-0">
                             ...
                             <p className="text-[9px] text-slate-400 font-bold uppercase mt-0.5">{file.provider === 'cloud' ? 'سحابي (Drive)' : 'تخزين محلي'}</p>
                          </div>
                        </div>
                    ...
                    <div className="py-12 text-center border-2 border-dashed border-slate-100 rounded-[2rem]">
                       ...
                       <p className="text-[10px] text-slate-400 font-bold italic">لا توجد ملفات</p>
                    </div>
                  )}
                </div>
              </section>
              ...
              <section className="space-y-4">
                <h4 className="text-sm font-black text-slate-800 dark:text-white flex items-center gap-2"><Info size={18} className="text-blue-500" /> الوصف</h4>
            ...
            {/* Preview Panel - Re-engineered with Google Docs Viewer */}
            {previewFile && (
              <div className="flex-1 bg-slate-200 dark:bg-slate-950 relative flex flex-col animate-in slide-in-from-left duration-500">
                 ...
                 <div className="flex-1 w-full h-full p-4 relative">
                    {previewFile.type === 'pdf' || previewFile.name.toLowerCase().endsWith('.pdf') ? (
                      <div className="w-full h-full rounded-2xl bg-white shadow-2xl relative overflow-hidden">
                        ...
                        {/* الخلفية تظهر في حال فشل القارئ في التحميل لسبب أمني */}
                        <div className="absolute inset-0 flex flex-col items-center justify-center bg-slate-100 p-12 text-center z-0">
                           <div className="p-6 bg-white rounded-[3rem] shadow-xl border border-slate-100 max-w-sm">
                             <Globe size={64} className="mx-auto text-blue-500 mb-6 animate-pulse" />
                             <h5 className="text-xl font-black text-slate-800 mb-3">يتم الآن تجهيز المخطط...</h5>
                             <p className="text-xs text-slate-500 mb-8 leading-relaxed">
                               نحن نستخدم محرك Google Docs لمعالجة ملف الـ PDF بأمان. إذا استغرق الأمر وقتاً طويلاً، يمكنك فتح الرابط مباشرة.
                             </p>
                             <div className="flex flex-col gap-3">
                               <button 
                                onClick={() => window.open(previewFile.url, '_blank')}
                                className="w-full py-4 bg-blue-600 text-white rounded-2xl font-black text-sm shadow-lg hover:bg-blue-700"
                               >
                                  فتح الرابط المباشر للمخطط
                               </button>
                             </div>
                           </div>
                        </div>

                        {/* الإطار الذي يعرض "نسخة الويب" من الـ PDF */}
                        <iframe 
                          src={displayUrl || ''} 
                      ...
                      <div className="w-full h-full flex items-center justify-center bg-slate-300 dark:bg-slate-900 rounded-2xl overflow-hidden p-6">
                         <img 
                      ...
                      <div className="w-full h-full flex flex-col items-center justify-center space-y-6">
                         <div className="p-12 bg-white dark:bg-slate-900 rounded-[3rem] shadow-xl text-center">
                            ...
                            <button 
                              onClick={() => handleDownload(previewFile)}
                              className="px-10 py-4 bg-blue-600 text-white rounded-2xl font-black text-sm shadow-xl hover:bg-blue-700"
                            >
                              تحميل الملف للمراجعة
                            </button>
                         </div>
                      </div>
                    )}
      ...
      <ConfirmationModal 
        ...
        title="حذف المهمة؟"
        message="سيتم مسح هذه المهمة وكافة مرفقاتها نهائياً."
      />
    </>
  );
};
